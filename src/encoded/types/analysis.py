...
class Analysis(Item):
    ...
    @calculated_property(define=True, schema={
        ...
        "description": "A list of quality metrics associated with files generated by this analysis.",
        "comment": "Do not submit. This field is calculated from files in this analysis.",
        "type": "array",
        "notSubmittable": True,
        "items": {
    ...
    def quality_metrics(self, request, files):
        file_objects = [
            request.embed(
                f,
                '@@object_with_select_calculated_properties?field=quality_metrics&field=biological_replicates&field=@id'
            )
            for f in files
        ]
