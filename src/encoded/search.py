...
class SearchViews:
    '''Search View based on Pyramid Docs'''

    def __init__(self, context, request, return_generator=False):
        self.context = context
        self.es_index = '_all'
        self.principals = effective_principals(request)
        self.registry_es = request.registry[ELASTIC_SEARCH]
        self.registry_types = request.registry[TYPES]
        self.request = request
        self.return_generator = return_generator
        self.search_audit = request.has_permission('search_audit')
        ...
        bad_doc_types = []
        for doc_type in doc_types:
            if doc_type in self.registry_types:
                good_doc_types.append(self.registry_types[doc_type].name)
            else:
                bad_doc_types.append(doc_type)
        self.doc_types = sorted(good_doc_types)
        self.bad_doc_types = bad_doc_types
        if not self.bad_doc_types:
            self._post_init()

    def _post_init(self):
        searchterm_specs = self.request.params.getall('searchTerm')
        searchterm_only = urlencode(
            [("searchTerm", searchterm) for searchterm in searchterm_specs]
        )
        if searchterm_only:
            clear_qs = searchterm_only
        else:
            clear_qs = urlencode([("type", typ) for typ in self.doc_types])
        self.clear_filters = "{}{}".format(
            self.request.route_path('search', slash='/'),
        ...
        if len(self.doc_types) == 1:
            only_doc_type = self.doc_types[0]
            reg_type_factory = self.registry_types[only_doc_type].factory
            result_views = self._get_views(reg_type_factory)

        self.result_obj = SearchResult(
            self.clear_filters,
            self.filters,
            self.request.route_path('jsonld_context'),
            self.search_base,
        )
        self.result_obj.add_views(result_views)

    def _get_filters(self):
        filters = []
        types = self.registry_types
    ...
    def _get_views(self, reg_type_factory):
        views = []
        views.append({
    ...
    def search(self):
        ...
        '''Base Search'''
        if self.bad_doc_types:
            msg = "Invalid type: {}".format(', '.join(self.bad_doc_types))
            raise HTTPBadRequest(explanation=msg)
        search_fields, highlights = get_search_fields(self.request, self.doc_types)
        query = get_filtered_query(
            self.search_term,
            search_fields,
            sorted(list_result_fields(self.request, self.doc_types)),
            self.principals,
            self.doc_types
        )
        if self.search_term == '*':
        ...
        schemas = [
            self.registry_types[doc_type].schema
            for doc_type in self.doc_types
        ]
        columns = list_visible_columns_for_schemas(self.request, schemas)
        self.result_obj.add_columns(columns)
        set_sort_order(
            self.request,
            self.search_term,
            self.registry_types,
        ...
        used_filters = set_filters(self.request, query, self.result_obj.result)
        facets = [('type', {'title': 'Data Type'})]
        if (len(self.doc_types) == 1 and
                'facets' in self.registry_types[self.doc_types[0]].schema):
            facets.extend(
                self.registry_types[self.doc_types[0]].schema['facets'].items()
            )
        for audit_facet in audit_facets:
            if (self.search_audit and
                    'group.submitter' in self.principals
                    or 'INTERNAL_ACTION' not in audit_facet[0]):
                facets.append(audit_facet)
        query['aggs'] = set_facets(facets, used_filters, self.principals, self.doc_types)
        query = sort_query(query)
        do_scan = self.size is None or self.size > 1000
        if not self.request.params.get('type') or 'Item' in self.doc_types:
            self.es_index = '_all'
        else:
            self.es_index = [
        ...
        if do_scan:
            es_results = self.registry_es.search(
                body=query, index=self.es_index, search_type='query_then_fetch'
            )
        else:
            es_results = self.registry_es.search(
                body=query,
        ...
        self.result_obj.add_total(es_results['hits']['total'])
        schemas = (self.registry_types[item_type].schema for item_type in self.doc_types)
        self.result_obj.add_facets(
            format_facets(
                es_results,
                facets,
                used_filters,
        ...
        self.result_obj.add_update(
            search_result_actions(self.request, self.doc_types, es_results)
        )
        if self.size is not None and self.size < total:
            params = [(k, v) for k, v in self.request.params.items() if k != 'limit']
            params.append(('limit', 'all'))
            self.result_obj.add_all('%s?%s' % (
                self.request.resource_path(self.context), urlencode(params)
            ))
        # Returns
        if not total:
            self.request.response.status_code = 404
            self.result_obj.add_notification('No results found')
            self.result_obj.add_graph([])
            return self.result_obj.result if not self.return_generator else []
        self.result_obj.add_notification('Success')
        if not do_scan:
        ...
        graph = format_results(self.request, hits, self.result_obj.result)
...
class SearchResult(object):
    '''To Organize building search result dict'''
    # pylint: disable=missing-docstring

    def __init__(self, clear_filters, filters, jsonld_context, search_base):
        self.result = {
    ...
    def add_notification(self, notification):
        self.result['notification'] = notification

    def add_graph(self, graph):
        self.result['@graph'] = graph

    def add_update(self, update_obj):
        self.result.update(update_obj)

    def add_all(self, all_str):
        self.result['all'] = all_str


def iter_search_results(context, request):
    search_view = SearchViews(context, request, return_generator=True)
