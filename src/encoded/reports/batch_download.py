...
from encoded.reports.constants import SERIES_BATCH_DOWNLOAD_COLUMN_TO_FIELDS_MAPPING_FOR_FILES
...
from encoded.reports.metadata import MultipleSeriesMetadataReport
...
class SeriesBatchDownloadForFiles(BatchDownload):

    def _get_column_to_fields_mapping(self):
        return SERIES_BATCH_DOWNLOAD_COLUMN_TO_FIELDS_MAPPING_FOR_FILES
...
def _get_series_batch_download(context, request):
    multiple_series_batch_download = MultipleSeriesBatchDownload(request)
    return multiple_series_batch_download.generate()
