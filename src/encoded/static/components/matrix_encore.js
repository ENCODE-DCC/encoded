...
/**
 * Limit the assays to display on the horizontal axis of the main ENCORE matrix to the following:
 */
...
/**
 * Limit the assays to display on the horizontal axis of the inset RNA-seq matrix to the following:
 */
...
/**
 * Limit the biosample term names to display within each assay to the following:
 */
...
const convertEncoreToDataTable = (context, displayedAssays, rowCategoryFilterText) => {
    ...
    // for the clicked column, not all assay_titles that might be in the query string.
    ...
    context.matrix.x[colCategory].buckets.forEach((colCategoryItem) => {
        if (displayedAssays.includes(colCategoryItem.key) && doesRowDataExist(context, colCategoryItem.key, colCategory, colSubCategory, rowCategory)) {
            ...
            colSubCategoryBuckets.forEach((colSubCategoryItem) => {
                ...
                // its value is another object containing the biosample term names as keys, with
                // their values being the column number for that assay/biosample-term-name pair.
            ...
            colCategoryData.push({
                header: (
                    <a href={`${colCategoryUrl}&${colCategoryItem.key === 'RNA Bind-n-Seq' ? `${colSubCategory}=${encoding.encodedURIComponent('cell-free sample')}` : displayedColSubCategoryQuery}`}>
                        {colCategoryItem.key}
                    </a>
                ),
    ...
    if (rowCategory === 'target.label') {
        ...
        displayedColSubCategoryQuery += `&${colSubCategory}=${encoding.encodedURIComponent('cell-free sample')}`;
    }
    rowCategoryBuckets.forEach((rowCategoryItem) => {
        ...
        if (hasTermName) {
            ...
            rowCategoryUrl = `${baseUrlWithoutColCategoryType}`;
        }
        ...
        cells[0] = { header: <a href={`${rowCategoryUrl}&${colCategoryQuery}&${displayedColSubCategoryQuery}`}>{hasTermName ? rowCategoryItem.key : '(all)'}</a> };
