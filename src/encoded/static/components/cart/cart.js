 ...
 * @param {string} countingField Field of array to count to find total file count
...
const addToAccumulatingFacets = (accumulatingResults, currentResults, facetFields, countingField) => {
    ...
    if (currentResults['@graph'] && currentResults['@graph'].length > 0) {
        ...
        // an object to make finding each entry in the copy by field easy.
 ...
 * experiments with files matching a criteria. So ultimately this has to *appear* as a file
 * search. But we can't do a file search because batch_download has specific criteria related to
 * experiment statuses.
 */
class FileFacets extends React.Component {
    ...
    retrieveFileFacetTemplate() {
        // Need to see how file facets look so we can build one from experiment searches, so we can
        // use some of the same rendering functions as regular facets.
    ...
    retrieveFileFacets() {
        ...
        // adding the totals together to form the final facet. Also count the total number of
        // viewable cart elements.
 ...
 */
const ElementCountArea = ({ count, viewableElementCount, name, namePlural }) => {
    if (count > 0) {
        return (
            <div className="cart__element-count">
                <span>{count}&nbsp;{count === 1 ? name : namePlural} in cart</span>
...
ElementCountArea.propTypes = {
    ...
    /** Singular name of elements being displayed */
    name: PropTypes.string.isRequired,
    /** Plural name of elements being displayed */
...
const createInitialCartState = () => {
    const newState = {
        ...
        fileFacets: {},
        /** Count of elements viewable at user's access level */
        ...
        return (
            <div className={itemClass(context, 'view-item')}>
                ...
                <Panel addClasses="cart__result-table">
                    ...
                    : null}
                    <ElementCountArea count={cartElements.length} viewableElementCount={this.state.viewableElementCount} name="dataset" namePlural="datasets" />
                    <PanelBody>
