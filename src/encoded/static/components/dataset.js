...
function donorDiversity(dataset) {
            ...
            if (replicates.length) {
                ...
                replicates.every((replicate) => {
                    if (replicate.library && replicate.library.status !== 'deleted' &&
                        ...
                        const donorAtId = replicate.library.biosample.donor['@id'];

                        // If we haven't yet seen this donor @id, add it to our collection
                        if (donorAtIdCollection.indexOf(donorAtId) === -1) {
                            donorAtIdCollection.push(donorAtId);
                        }
    ...
    console.log('DIVERSITY: %s', diversity);
    return diversity;
}
