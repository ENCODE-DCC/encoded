...
const Encyclopedia = (props, context) => {
    const annotation = props.context.filters.filter((f) => f.field === 'annotation_type').map((f) => f.term);
    const assembly = props.context.filters.filter((f) => f.field === 'assembly').map((f) => f.term)[0];
    const organism = ASSEMBLY_DETAILS[assembly].species;
    const searchBase = '?type=File&annotation_type=candidate+Cis-Regulatory+Elements&assembly=GRCh38&file_format=bigBed&file_format=bigWig';
    ...
    annotationTypes.forEach((type) => {
        if (props.context.facets.filter((f) => f.field === 'annotation_type')[0] && props.context.facets.filter((f) => f.field === 'annotation_type')[0].terms.filter((t) => t.key === type)[0]) {
