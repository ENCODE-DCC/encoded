...
const convertExperimentToDataTable = (context, getRowCategories, getRowSubCategories, mapRowCategoryQueries, mapSubCategoryQueries, expandedRowCategories, expanderClickHandler, clearClassifications, stageFilter) => {
    ...
    const testRowCategoryData = JSON.parse(JSON.stringify(rowCategoryData));
    rowCategoryData.forEach((row, rowIdx) => {
        ...
        testRowCategoryData[rowIdx].biosample_summary.buckets = newRowBuckets;
    });

    // loop through row data to sort buckets by sorted mouse stage/age
    const newRowCategoryData = JSON.parse(JSON.stringify(testRowCategoryData));
    testRowCategoryData.forEach((row, rowIdx) => {
        const rowAgeArray = [];
        row.biosample_summary.buckets.forEach((bucket) => {
    ...
    const header = [
        ...
        return { header: <a className="sub" href={`${context.search_base}&${categoryQuery}&${subCategoryQuery}`}>{colInfo.subcategory} {colInfo.category.replace('Histone ', '')}</a> };
    }));
    ...
    const dataTable = newRowCategoryData.reduce((accumulatingTable, rowCategoryBucket) => {
        ...
        const subcategoryRows = filteredRowSubcategoryBuckets.map((rowSubcategoryBucket) => {
            ...
            console.log(maleSubcategoryBucketKey);
            console.log(femaleSubcategoryBucketKey);
...
const MouseStageButton = (props) => {
    ...
    return (
        <button
            id={idString}
            className={`legend-option ${idString} ${activeClass ? 'active' : ''}`}
            onClick={onClick}
...
class MatrixPresentation extends React.Component {
    ...
    selectMouseDevelopmentStage(e) {
        ...
        if (this.state.developmentStageClick.indexOf(e.target.id) > -1) {
            const newSelections = this.state.developmentStageClick.filter(stage => stage !== e.target.id);
            this.setState({ developmentStageClick: newSelections });
        // selected a particular age so need to disable the stage
        } else if (e.target.id.split(' ').length > 1) {
            let newSelections = [];
            if (e.target.id.includes('embryo')) {
                newSelections = this.state.developmentStageClick.filter(stage => stage !== 'embryo');
            } else if (e.target.id.includes('postnatal')) {
    ...
    render() {
        ...
        // Apply filter from buttons or drop-down
