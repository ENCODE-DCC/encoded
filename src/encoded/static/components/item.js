    ...
    var ItemEdit = module.exports.ItemEdit = React.createClass({
        render: function() {
            ...
            if (context['@type'][0].indexOf('_collection') !== -1) {  // add form
                ...
                form = <Form schema={this.props.schema} action={action} value={this.props.defaultValue} method="POST" />;
            } else {  // edit form
                title = 'Edit ' + title;
                var url = this.props.context['@id'] + '?frame=edit';
                action = this.props.context['@id'];
                form = <FetchedData fetched_prop_name="value" Component={Form} url={url} schema={this.props.schema} action={action} method="PUT" />;
            }
            return (
                <div className={itemClass}>
                    <header className="row">
