...
#@upgrade_step('pbc_quality_metric', '2', '3')
'''@upgrade_step('generic_quality_metric', '2', '3')
def quality_metric_2_3(value, system):
    # http://redmine.encodedcc.org/issues/3936
    if 'quality_metric_of' not in value or \
       len(value['quality_metric_of']) == 0:
        conn = system['registry'][CONNECTION]
        step_run = conn.get_by_uuid(value['step_run'])
        output_files = conn.get_rev_links(step_run.model, 'step_run', 'File')
        value['quality_metric_of'] = list(set([str(uuid) for uuid in output_files]))
'''