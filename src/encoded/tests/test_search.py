...
def test_reference_epigenome_matrix_view(workbook, testapp):
    res = testapp.get('/reference-epigenome-matrix/?type=Experiment&related_series.@type=ReferenceEpigenome&replicates.library.biosample.donor.organism.scientific_name=Mus+musculus').json
    assert res['@type'] == ['ReferenceEpigenomeMatrix']
    assert res['@id'] == '/reference-epigenome-matrix/?type=Experiment&related_series.@type=ReferenceEpigenome&replicates.library.biosample.donor.organism.scientific_name=Mus+musculus'
    assert res['@context'] == '/terms/'
    assert res['notification'] == 'Success'
    assert res['title'] == 'Reference Epigenome Matrix'
    assert res['total'] > 0
    assert 'filters' in res
    assert 'matrix' in res
    assert res['matrix']['x']['group_by'] == ['assay_title', 'target.label']
    assert res['matrix']['x']['label'] == 'Assay'
    assert res['matrix']['y']['group_by'] == [
        'biosample_ontology.classification', 'biosample_ontology.term_name']
    assert res['matrix']['y']['label'] == 'Biosample'
