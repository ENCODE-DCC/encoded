...
def test_visualize(submitter_testapp, workbook):
    expected = {
        'GRCh38': {
            ...
            'Quick View': '/search/?type=File&assembly=GRCh38&dataset=/experiments/ENCSR000AEN/&file_format=bigBed&file_format=bigWig&status=released&status=in+progress#browserpane',
            'UCSC': 'http://genome.ucsc.edu/cgi-bin/hgTracks?hubClear=http://localhost/experiments/ENCSR000AEN/@@hub/hub.txt&db=hg38'
        },
        'hg19': {
            'Quick View': '/search/?type=File&assembly=hg19&dataset=/experiments/ENCSR000AEN/&file_format=bigBed&file_format=bigWig&status=released&status=in+progress#browserpane',
