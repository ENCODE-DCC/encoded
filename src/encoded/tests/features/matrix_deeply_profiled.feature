...
Feature: Matrix
    ...
    Scenario: Matrix
        When I visit "/deeply-profiled-matrix/?type=Experiment&control_type!=*&status=released"
        And I wait for the content to load
        Then the title should contain the text "Deeply Profiled Cell Lines"       
        And I should see 0 elements with the css selector "#deeplyProfiled:checked"
