...
def test_target_upgrade(upgrader, target_1_upgrade):
    value = upgrader.upgrade('target', target_1_upgrade, target_version='2')
    assert value['schema_version'] == '2'
    assert value['status'] == 'current'


def test_target_investigated_as_upgrade(upgrader, target_2_upgrade):
    value = upgrader.upgrade('target', target_2_upgrade, target_version='3')
    assert value['schema_version'] == '3'
    assert value['investigated_as'] == ['transcription factor']


def test_target_investigated_as_upgrade_tag(upgrader, target_2_upgrade):
    target_2_upgrade['label'] = 'eGFP'
    value = upgrader.upgrade('target', target_2_upgrade, target_version='3')
    assert value['schema_version'] == '3'
    assert value['investigated_as'] == ['tag']


def test_target_investigated_as_upgrade_recombinant(upgrader, target_2_upgrade):
    target_2_upgrade['label'] = 'eGFP-test'
    value = upgrader.upgrade('target', target_2_upgrade, target_version='3')
    assert value['schema_version'] == '3'
...
def test_target_upgrade_status_5_6(upgrader, target_5_upgrade):
    value = upgrader.upgrade(
        'target', target_5_upgrade, current_version='5', target_version='6')
    assert value['schema_version'] == '6'
    assert value['status'] == 'current'


def test_target_upgrade_remove_histone_modification_6_7(upgrader, target_6_upgrade):
    value = upgrader.upgrade(
        'target', target_6_upgrade, current_version='6', target_version='7')
