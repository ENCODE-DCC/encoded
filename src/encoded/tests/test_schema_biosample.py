...
def test_biosample_mouse_life_stage_fail(testapp, biosample_data2):
    biosample_data2['mouse_life_stage'] = 'adult'
    testapp.post_json('/biosample', biosample_data2, status=422)
...
def test_biosample_human_post_synchronization_fail(testapp, biosample_data2):
    biosample_data2['post_synchronization_time'] = '10'
    biosample_data2['post_synchronization_time_units'] = 'hour'
    testapp.post_json('/biosample', biosample_data2, status=422)
...
def test_biosample_human_whole_organism_fail(testapp, biosample_data2, whole_organism):
    biosample_data2['biosample_ontology'] = whole_organism['uuid']
    testapp.post_json('/biosample', biosample_data2, status=422)


def test_alt_accession_ENCBS_regex(testapp, biosample_data2):
    bio = testapp.post_json('/biosample', biosample_data2).json['@graph'][0]
...
def test_biosample_organoid_success(testapp, biosample_data2, organoid):
    biosample_data2['biosample_ontology'] = organoid['uuid']
    testapp.post_json('/biosample', biosample_data2, status=201)


def test_biosample_post_diffentiation_props(testapp, biosample_data2, organoid):
    biosample_data2['biosample_ontology'] = organoid['uuid']
    bio = testapp.post_json('/biosample', biosample_data2).json['@graph'][0]
    res = testapp.patch_json(
        bio['@id'],
