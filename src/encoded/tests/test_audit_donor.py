...
def test_audit_donor_dbxref(testapp, fly_donor_201):
    testapp.patch_json(fly_donor_201['@id'], {'dbxrefs': []}),
    res = testapp.get(fly_donor_201['@id'] + '@@index-data')
...
def test_audit_donor_external_ids(testapp, fly_donor_201):
    testapp.patch_json(fly_donor_201['@id'], {'dbxrefs': [], 'external_ids': []}),
    res = testapp.get(fly_donor_201['@id'] + '@@index-data')
    errors = res.json['audit']
    errors_list = []
    for error_type in errors:
