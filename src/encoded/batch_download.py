...
def is_file_visualizable(file):
    ...
    """
    return (
            (file.get('file_format') == 'bigWig' or file.get('file_format') == 'bigBed') and
            file.get('file_format_type') not in ['bedMethyl', 'bedLogR', 'idr_peak', 'tss_peak', 'pepMap', 'modPepMap'] and
            file.get('status') in ['released', 'in progress', 'archived']
    )
...
def metadata_tsv(context, request):
    ...
    visualizable_only = 'true' in param_list.get('visualizable', [])
