...
_tsv_mapping_publicationdata = OrderedDict([
    ('File accession', ['files.title']),
    ('File dataset', ['files.dataset.accession']),
    ('File type', ['files.file_format']),
    ('File format', ['files.file_type']),
...
def _get_publicationdata_metadata(request):
    ...
    if file_ids:
        # For each file, load the file object and use its properties to build each row of the table.
        for file_id in file_ids:
            ...
            # Build the row's data; much sync with _tsv_mapping_publicationdata.
            row = [
                file.get('title', ''),
                dataset_accession,
...
def _batch_download_publicationdata(request):
    ...
    # Retrieve the files property of the requested object.
