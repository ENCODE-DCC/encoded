...
def audit_temp_unique_array_items(value, system):
    ...
    if 'constructs' in value:
        if len(value['constructs']) != len(set(value['constructs'])):
            detail = 'Duplicated constructs in {}'.format(value['@id'])
            yield AuditFailure('duplicated constructs', detail, level='DCC_ACTION')

    if 'rnais' in value:
        if len(value['rnais']) != len(set(value['rnais'])):
            detail = 'Duplicated rnais in {}'.format(value['@id'])
            yield AuditFailure('duplicated rnais', detail, level='DCC_ACTION')

    if 'talens' in value:
        if len(value['talens']) != len(set(value['talens'])):
            detail = 'Duplicated talens in {}'.format(value['@id'])
            yield AuditFailure('duplicated talens', detail, level='DCC_ACTION')

    if 'treatments' in value:
        if len(value['treatments']) != len(set(value['treatments'])):
            detail = 'Duplicated treatments in {}'.format(value['@id'])
            yield AuditFailure('duplicated treatments', detail, level='DCC_ACTION')

    if 'protocol_documents' in value:
        if len(value['protocol_documents']) != len(set(value['protocol_documents'])):
            detail = 'Duplicated documents in {}'.format(value['@id'])
            yield AuditFailure('duplicated bio docs', detail, level='DCC_ACTION')

    if 'dbxrefs' in value:
        if len(value['dbxrefs']) != len(set(value['dbxrefs'])):
            detail = 'Duplicated dbxrefs in {}'.format(value['@id'])
            yield AuditFailure('duplicated bio dbxrefs', detail, level='DCC_ACTION')

    if 'pooled_from' in value:
        if len(value['pooled_from']) != len(set(value['pooled_from'])):
            detail = 'Duplicated pooled_from in {}'.format(value['@id'])
            yield AuditFailure('duplicated pooled_from', detail, level='DCC_ACTION')

    if 'depleted_in_term_name' in value:
        if len(value['depleted_in_term_name']) != len(set(value['depleted_in_term_name'])):
            detail = 'Duplicated depleted_in_term_name in {}'.format(value['@id'])
            yield AuditFailure('duplicated depleted_in', detail, level='DCC_ACTION')

    if 'depleted_in_term_id' in value:
        if len(value['depleted_in_term_id']) != len(set(value['depleted_in_term_id'])):
            detail = 'Duplicated depleted_in_term_id in {}'.format(value['@id'])
            yield AuditFailure('duplicated depleted_in', detail, level='DCC_ACTION')
            return
