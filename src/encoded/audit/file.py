...
def audit_temp_unique_array_items(value, system):
    ...
    if 'controlled_by' in value:
        if len(value['controlled_by']) != len(set(value['controlled_by'])):
            detail = 'Duplicated files in controlled_by of {}'.format(value['@id'])
            yield AuditFailure('duplicated controlled_by', detail, level='DCC_ACTION')

    if 'derived_from' in value:
        if len(value['derived_from']) != len(set(value['derived_from'])):
            detail = 'Duplicated files in derived_from of {}'.format(value['@id'])
            yield AuditFailure('duplicated derived_from', detail, level='DCC_ACTION')

    if 'supercedes' in value:
        if len(value['supercedes']) != len(set(value['supercedes'])):
            detail = 'Duplicated files in supercedes of {}'.format(value['@id'])
            yield AuditFailure('duplicated supercedes', detail, level='DCC_ACTION')

    if 'dbxrefs' in value:
        if len(value['dbxrefs']) != len(set(value['dbxrefs'])):
            detail = 'Duplicated dbxrefs in {}'.format(value['@id'])
            yield AuditFailure('duplicated file dbxrefs', detail, level='DCC_ACTION')

    if 'file_format_specifications' in value:
        if len(value(['file_format_specifications'])) != len(set(value['file_format_specifications'])):
            detail = 'Duplicated specifications in {}'.format(value['@id'])
            yield AuditFailure('duplicated specifications', detail, level='DCC_ACTION')

    if 'flowcell_details' in value:
        if len(value['flowcell_details']) != len(set(value['flowcell_details'])):
            detail = 'Duplicated flowcell_details in {}'.format(value['@id'])
            yield AuditFailure('duplicated flowcells', detail, level='DCC_ACTION')
            return
