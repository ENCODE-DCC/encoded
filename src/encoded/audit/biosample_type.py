...
def audit_term(value, system):
    ...
    if term_id not in ontology:
        detail = 'BiosampleType {} specifies a term_id {} that is not part of the {} ontology.'.format(
          audit_link(value['accession'], value['@id']),
          term_id,
          term_id.split(':', 1)[0]
          )
    ...
    if (ontology_term_name != term_name
        ...
        detail = ('BiosampleType {} has a mismatch between'
                  ' term_id {} and term_name {},'
                  ' ontology term_name for term_id {} is'
                  ' {}.').format(
                      audit_link(value['accession'], value['@id']),
                      term_id,
                      term_name,
