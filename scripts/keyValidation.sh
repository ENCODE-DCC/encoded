#!/bin/bash
                ...
                sudo -iu encoded bin/aws --profile encoded-files-upload s3 ls s3://$bucket/
                if [ $? -ne 0 ]; then
                        echo "ERROR: exit status 1 Cannot list s3 bucket: your keys may be incorrect or you do not have permissions for that bucket."
                        exit
                fi
                        sudo -iu encoded bin/aws --profile encoded-files-upload s3 cp s3://$bucket/testDown.jpg .
                if [ $? -ne 0 ]; then
                        ...
                        sudo -iu encoded bin/aws --profile encoded-files-upload s3 ls s3://$bucket/
                        sudo -iu encoded bin/aws --profile encoded-files-upload s3 mv testDown.jpg s3://$bucket/testUp.jpg
                if [ $? -ne 0 ]; then
