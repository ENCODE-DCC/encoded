...
if [ $indexerOutput == "indexing" ];then
	...
	if [ $input == "no" ];then
		...
		time python3 ~/Documents/ENCODE-DCC-encoded/Master_ENCODE-DCC-encoded/ENCODE-DCC-encoded/pyencoded-tools/ENCODE_patch_set.py --key test --accession 4c911ae3-9007-4777-9680-2c526ab8fdbb --field aliases:encode --data "test:Preformance" --update; python3 stress.py $URL_PREF;
 		fullExit
	fi
