...
if [ -f "$encoded_app_is_installed_file_flag" ]; then
    # May run on restarting the machine
    ...
    if [ -f "$encoded_app_do_install_file_flag" ]; then
        ...
        $CC_DIR/es-install.sh NONE NONE NONE yes
        $CC_DIR/pg-install.sh off $ROLE $WALE_S3_PREFIX $PG_VERSION
        $CC_DIR/encd-install.sh $ROLE $ES_IP $ES_PORT $REGION_INDEX $APP_WORKERS
        $CC_DIR/a2en-run.sh
        if [ "$ROLE" == "demo" ]; then
        ...
        # When building AMI
        echo 'POST AMI INSTALL: Pre install and set do install'
        sudo -u ubuntu git -C /home/ubuntu/encoded checkout -b $GIT_BRANCH $GIT_REMOTE/$GIT_BRANCH
        $CC_DIR/encd-preinstall.sh $GIT_REPO $GIT_REMOTE $GIT_BRANCH
