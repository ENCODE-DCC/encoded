...
PG_CONF_SRC="$ENCD_CC_DIR/conf-pg"
...
WALE_REQS="$ENCD_CC_DIR/requirements-wale.txt"
...
sudo -u root cp "$WALE_REQS" "$WALE_DIR/requirements-wale.txt"
sudo -u root chown postgres:postgres "$WALE_DIR/requirements-wale.txt"
...
sudo -H -u postgres "$WALE_BIN/pip" install -r "$WALE_DIR/requirements-wale.txt"
