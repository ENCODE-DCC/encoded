...
if [ ! -f "$encd_is_installed_flag" ]; then
    ...
    echo -e "\n\t$ENCD_INSTALL_TAG $(basename $0) Skipping for AMI build"
    exit 0
else
...
CLOUDWATCHMON_REQS='requirements-cloudwatchmon.txt'
CLOUDWATCHMON_VENV="$CLOUDWATCHMON_HOME/.cwm-pyenv"

sudo mkdir "$CLOUDWATCHMON_HOME"
sudo cp "$ENCD_CC_DIR/$CLOUDWATCHMON_REQS" "$CLOUDWATCHMON_HOME/$CLOUDWATCHMON_REQS"
